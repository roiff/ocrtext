...
--- @param url string @必填,接口地址 'http://ip:port/api/qrcode/',默认端口8099
function M.new(url)
    local obj = {}
    -- 确保临时文件存储的文件夹存在
    obj.path = userPath().."/res/qrcode/"
    obj.url = url
    do
    ...
    obj.img_path = obj.path.."qrcode.jpg"
    -- dialog(obj.img_path)
        ...
        if code == 200 then
            ...
            if #str > 0 then
                ...
                if points then
                    ...
                    logger:debug("识别二维码识别内容失败: "..response_body[1])
                    return
                end
            end
        else
            logger:error("识别二维码连接失败: "..response_body[1],self.img_path)
            return
        end
